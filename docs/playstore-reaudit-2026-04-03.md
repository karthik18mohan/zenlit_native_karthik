...
| Signup/onboarding legal acceptance | PARTIAL | MEDIUM | Sign-up is gated by checkbox before OTP send, and legal-consent screen forces explicit acceptance persisted to `legal_acceptances`. But sign-up checkbox state is not persisted/recorded itself and cannot be audited independently. | Persist acceptance event at first consent touchpoint or remove duplicate checkbox and rely on one mandatory, persisted legal-consent step with explicit wording. |
...
- **Code-complete for Play compliance:** **No** (due to two medium issues above).
