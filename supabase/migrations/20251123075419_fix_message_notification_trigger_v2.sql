/*
  # Fix Message Notification Trigger - Version 2

  1. Problem
    - Previous trigger referenced 'topic' and 'conversation_id' fields that don't exist
    - The messages table only has: id, sender_id, receiver_id, text, created_at, delivered_at, read_at

  2. Solution
    - Update trigger to use actual table fields
    - Remove references to non-existent fields
    - Simplify the data payload

  3. Changes
    - Remove conversation_id/topic references
    - Use only existing fields: id, sender_id, receiver_id, text
*/
...
BEGIN
  ...
  -- Check if conversation is muted (sender is in muted list)
  ...
  -- Create notification body (truncate if too long)
